...
ARID_DEF_SeatBelt_T SeatBelt_ARID_DEF; /* '<S2>/average_speed' */
...
void demo_Step(void)
{
  ...
  rtb_RelationalOperator_f = (rtb_RelationalOperator_c_tmp ==
    SeatBelt_ARID_DEF.UnitDelay_DSTATE);

  /* Logic: '<S2>/Logical Operator1' */
  SeatBelt_ARID_DEF.LogicalOperator1 = !rtb_RelationalOperator_f;

  /* S-Function (sdspcount2): '<S2>/Counter1' */
  if (MWDSP_EPH_F_B(SeatBelt_ARID_DEF.LogicalOperator1,
                    &SeatBelt_ARID_DEF.Counter1_RstEphState) != 0U) {
    SeatBelt_ARID_DEF.Counter1_Count = 0U;
  }

  if (MWDSP_EPH_F_B(rtb_RelationalOperator_f,
                    &SeatBelt_ARID_DEF.Counter1_ClkEphState) != 0U) {
    if (SeatBelt_ARID_DEF.Counter1_Count < 3) {
      SeatBelt_ARID_DEF.Counter1_Count++;
    } else {
      SeatBelt_ARID_DEF.Counter1_Count = 0U;
    }
  }
  ...
  /* MATLAB Function: '<S2>/average_speed' */
  SeatBelt_ARID_DEF.buffer[(sint32)SeatBelt_ARID_DEF.index - 1] = tmp_0;
  if (SeatBelt_ARID_DEF.index == 0.0) {
    r = 0.0;
  } else {
    r = fmod(SeatBelt_ARID_DEF.index, 5.0);
    if (r == 0.0) {
      r = 0.0;
    } else if (SeatBelt_ARID_DEF.index < 0.0) {
      r += 5.0;
    }
  }

  SeatBelt_ARID_DEF.index = r + 1.0;
  SeatBelt_ARID_DEF.count = fmin(SeatBelt_ARID_DEF.count + 1.0, 5.0);

  /* Update for UnitDelay: '<S2>/Unit Delay' */
  SeatBelt_ARID_DEF.UnitDelay_DSTATE = rtb_RelationalOperator_c_tmp;
   ...
   *  Store in Global RAM
   */
  SeatBelt_ARID_DEF.IgnitionKeySwitchCount = (uint8)((uint32)(((sint32)
    rtb_RelationalOperator > (sint32)SeatBelt_ARID_DEF.DelayInput1_DSTATE) ||
    ((sint32)rtb_RelationalOperator1 > (sint32)
     SeatBelt_ARID_DEF.DelayInput1_DSTATE_d)) +
    SeatBelt_ARID_DEF.IgnitionKeySwitchCount);
   ...
   */
  /* MATLAB Function: '<S2>/average_speed' */
  r = rt_roundd(((((SeatBelt_ARID_DEF.buffer[0] + SeatBelt_ARID_DEF.buffer[1]) +
                   SeatBelt_ARID_DEF.buffer[2]) + SeatBelt_ARID_DEF.buffer[3]) +
                 SeatBelt_ARID_DEF.buffer[4]) / SeatBelt_ARID_DEF.count);
  if (r < 65536.0) {
    if (r >= 0.0) {
      tmp_0 = (uint16)r;
    } else {
   ...
   *  Switch: '<S2>/Switch'
   */
  SeatBelt_ARID_DEF.AND_Logic = (tmp_0 > 5);
  SeatBelt_ARID_DEF.AND_Logic = ((rtb_RelationalOperator_tmp == ON) &&
    (rtb_RelationalOperator_f && (!rtb_RelationalOperator_c_tmp)) &&
    SeatBelt_ARID_DEF.AND_Logic);
   ...
   *  Store in Global RAM
   */
  SeatBelt_ARID_DEF.DelayInput1_DSTATE = rtb_RelationalOperator;
   ...
   *  Store in Global RAM
   */
  SeatBelt_ARID_DEF.DelayInput1_DSTATE_d = rtb_RelationalOperator1;

  /* End of Outputs for S-Function (fcgen): '<Root>/Function-Call Generator1' */

  /* Outport generated from: '<Root>/Out Bus Element1' */
  Rte_IWrite_demo_Step_SeatbeltIcon_SeatbeltIcon(SeatBelt_ARID_DEF.AND_Logic);
  ...
  Rte_IWrite_demo_Step_SeatbeltFastenError_SeatbeltFastenError
    (SeatBelt_ARID_DEF.Counter1_Count >= 1);
...
void demo_Init(void)
{
  ...
  SeatBelt_ARID_DEF.Counter1_ClkEphState = 5U;
  SeatBelt_ARID_DEF.Counter1_RstEphState = 5U;

  /* SystemInitialize for MATLAB Function: '<S2>/average_speed' */
  SeatBelt_ARID_DEF.index = 1.0;
